...

{
  ...
  python3 "$ROOT/tests/test_source_honesty.py" >/dev/null 2>&1 && echo "source-honesty tests: PASS" || echo "[WARN] source-honesty tests FAILED — see python3 tests/test_source_honesty.py"
  python3 "$ROOT/tests/test_neighborhood_gate.py" >/dev/null 2>&1 && echo "neighbourhood-gate tests: PASS" || echo "[WARN] neighbourhood-gate tests FAILED — see python3 tests/test_neighborhood_gate.py"
  python3 "$ROOT/tests/test_public_product_boundary.py" >/dev/null 2>&1 && echo "public-product-boundary tests: PASS" || echo "[WARN] public-product-boundary tests FAILED — see python3 tests/test_public_product_boundary.py"
